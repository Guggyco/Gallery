...
public protocol GalleryControllerDelegate: class {
    ...
    func galleryController(_ controller: GalleryController, createMeme images: [Image])
...
public class GalleryController: UIViewController, PermissionControllerDelegate {
    ...
    func setup() {
        ...
        EventHub.shared.createMeme = { [weak self] in
            if let strongSelf = self {
                strongSelf.delegate?.galleryController(strongSelf, createMeme: strongSelf.cart.images)
            }
        }
