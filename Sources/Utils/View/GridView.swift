...
class GridView: UIView {
    ...
    lazy var memeButton: UIButton = self.makeMemeButton()
    ...
    private func setup() {
        ...
        [bottomBlurView, doneButton, memeButton].forEach {
        ...
        memeButton.g_pin(on: .centerY)
        memeButton.g_pin(on: .right, constant: -90)
    ...
    private func makeMemeButton() -> UIButton {
        let button = UIButton(type: .system)
        button.setTitleColor(UIColor.white, for: UIControlState())
        button.setTitleColor(UIColor.lightGray, for: .disabled)
        button.titleLabel?.font = Config.Font.Text.regular.withSize(16)
        button.setTitle("meme", for: UIControlState())
